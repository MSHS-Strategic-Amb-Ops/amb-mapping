...
amb_mapping$LAST_ARRIVED <- last_arrived$LAST_ARRIVED[match(amb_mapping$DEPARTMENT_ID, last_arrived$DEPARTMENT_ID)]
amb_mapping <- amb_mapping %>%
  mutate(LAST_ARRIVED = as.character(LAST_ARRIVED)) %>%
  rename(CAMPUS = SITE,
         CAMPUS_SPECIALTY = DEPT_SPECIALTY_NAME) %>%
  arrange(CAMPUS, CAMPUS_SPECIALTY) %>%
  dplyr::select(CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT_NAME, Department_Clean, DEPARTMENT_ID, LAST_ARRIVED)

amb_mapping$total <- NULL
...
## Site Mapping (Grouper 17)
<span style='font-size: 18px;; color: #d80b8c'>**Epic Site-Specialty-Department Mapping**</span>
