...
amb_mapping_dept <- access_raw_tbl %>%
  ...
  summarise(LAST_ARRIVAL_DATE = max(CONTACT_DATE[DERIVED_STATUS_DESC == "Arrived"])) %>%
  arrange(SITE, DEPARTMENT_NAME) %>%
  collect() %>%
  mutate(LAST_ARRIVAL_DATE = as.character(LAST_ARRIVAL_DATE))
...
amb_mapping_prov <- amb_mapping_prov %>%
  ...
  dplyr::select(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, DEP_RPT_GRP_ELEVEN, PROV_NAME_WID, PROV_NAME_WID, NPI, PROV_ID, PROV_TYPE, TKT_SCH_YN, LAST_ARRIVAL_DATE)
